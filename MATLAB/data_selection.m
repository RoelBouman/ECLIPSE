...
%%
%Dependencies
% select_tube (to select tube if multiple are present)
...
if isempty(selection_mode)
    ...
    while check_label == 0 || check_label == 1 %used to create the warning
        ...
        elseif isa(Labels{1},'double') || isa(Labels{1},'logical')%if double instead of char or cell, first convert to cell using cellmat when comparing
...
if isfield(S(1), 'Tube')
    if isempty(selection_mode)
        ...
        [S, tubenr] = select_tube(S, selection_mode{4});
    end
