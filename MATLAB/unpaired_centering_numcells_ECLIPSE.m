function [S_mean,mean_choice] = unpaired_centering_numcells_ECLIPSE(S, center_mode)
%%
...
if center_mode == 1
    mean_control = mean_of_means(S(logical((Labels == 1).*trainset)));
    for l1 = 1:length(S)
        S_mean(l1).Data = S(l1).Data - repmat(mean_control,size(S(l1).Data,1),1);
    end
    mean_choice=mean_control;
elseif center_mode == 2
    median_control = mean_of_medians(S(logical((Labels == 1).*trainset)));
    for l1 = 1:length(S)
        S_mean(l1).Data = S(l1).Data - repmat(median_control,size(S(l1).Data,1),1);
    end
    mean_choice=median_control;
